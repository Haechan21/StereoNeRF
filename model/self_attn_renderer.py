...
class Renderer(nn.Module):
    ...
    def forward(self, viewdirs, feat, occ_masks):
        ...
        ## Get loss masking
        loss_mask = vis_mask.view(N, S, V)
        loss_mask = loss_mask.sum(dim=2) > 0  # [N_rays, N_samples]. should at least have 1 observation
        loss_mask = loss_mask.float().sum(dim=1) > 7  # [N_rays]. should at least have 8 valid observation on the ray
